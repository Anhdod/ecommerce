...
export default function CheckoutPage({ user }) {
  ...
  const loadAddresses = async () => {
    try {
      ...
      if (loadedAddresses.length && !selectedAddressId) {
        const defaultAddress = loadedAddresses.find((address) => address.defaultAddress === true);
        setSelectedAddressId((defaultAddress || loadedAddresses[0]).id);
      }
    } catch (error) {
          ...
          <div className="checkout-form-column">
            <section className="checkout-section">
              ...
              {addresses.length ? (
                ...
                <div className="checkout-no-address"><MapPin size={26} /><span>Chưa có địa chỉ giao hàng.</span><Link to="/addresses">Thêm địa chỉ</Link></div>
              )}
            </section>
            ...
            <div className="checkout-summary-items">
              {checkoutPreview?.cart?.items?.map((item) => (
                <div className="checkout-summary-item" key={item.cartItemId}>
                  ...
                  <div><strong>{item.productName}</strong><small>{item.selectedColor ? `Màu ${item.selectedColor} · ` : ''}{currency(item.price)}</small></div>
