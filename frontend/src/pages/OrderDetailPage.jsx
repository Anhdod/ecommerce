...
import { ArrowLeft, Check, CircleDollarSign, Clock3, CreditCard, Edit3, MapPin, Package, PackageCheck, Phone, RefreshCw, Save, ShieldCheck, ShoppingBag, Truck, XCircle } from 'lucide-react';
...
const progressStatuses = ['PENDING', 'CONFIRMED', 'SHIPPING', 'DELIVERED'];
...
export default function OrderDetailPage({ user }) {
  ...
  const updateTrackingCode = () => runAction(
    () => api(`/api/orders/${id}/tracking?trackingCode=${encodeURIComponent(trackingCode)}`, { method: 'PUT' }),
    'Đã cập nhật mã vận đơn',
    'Không cập nhật được mã vận đơn'
  );
        ...
        <div className="order-detail-layout">
          <div className="order-detail-main">
              ...
              <div className="order-detail-items">
                {order.items?.map((item) => (
                  <article key={item.orderItemId}>
                    ...
                    <div><Link to={`/products/${item.productId}`}>{item.productName}</Link><span>{item.selectedColor ? `Màu ${item.selectedColor} · ` : ''}{currency(item.price)} · Số lượng {item.quantity}</span></div>
            ...
            {isManager && (
              <section className="order-detail-section manager-order-tools">
                <div className="detail-section-heading"><div><h2>Công cụ quản lý</h2><p>Cập nhật trạng thái xử lý và mã vận đơn.</p></div><ShieldCheck size={18} /></div>
                <div className="manager-tools-grid">
                  <label>Trạng thái<select value={status} onChange={(event) => setStatus(event.target.value)}>{orderStatuses.map((item) => <option key={item} value={item}>{statusInfo[item].label}</option>)}</select><button type="button" onClick={updateStatus} disabled={actionLoading}>Cập nhật</button></label>
                  <label>Mã vận đơn<input value={trackingCode} onChange={(event) => setTrackingCode(event.target.value)} placeholder="Nhập mã tracking" /><button type="button" onClick={updateTrackingCode} disabled={actionLoading}>Lưu mã</button></label>
                </div>
              </section>
            )}
          ...
          <aside className="order-detail-sidebar">
            ...
            <div className="order-detail-actions">
              ...
              {payment?.status === 'PENDING' && payment?.paymentMethod === 'BANK_TRANSFER' && order.status === 'PENDING' && <Link className="pay-order-button" to={`/checkout/payment/${id}`}><CircleDollarSign size={17} /> Hướng dẫn chuyển khoản</Link>}
