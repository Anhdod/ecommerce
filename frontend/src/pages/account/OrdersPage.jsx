...
import { ArrowRight, Check, Clock3, MapPin, Package, PackageOpen, RefreshCw, Save, Search, ShieldCheck, ShoppingBag, Truck, XCircle } from 'lucide-react';
...
export default function OrdersPage({ user }) {
  ...
  const updateShipping = async (orderId) => {
    ...
    if (!carrier || !trackingCode) {
      showMessage('Vui lòng nhập hãng vận chuyển và mã vận đơn');
      return;
    }
    try {
      setUpdatingId(orderId);
      await api(`/api/orders/${orderId}/shipping`, {
        method: 'PUT',
        body: { carrier, trackingCode, estimatedDeliveryDate: null },
      });
      showMessage('Đã cập nhật thông tin vận chuyển');
      await loadOrders();
    } catch (error) {
      showMessage(error?.message || 'Không cập nhật được thông tin vận chuyển');
    } finally {
      setUpdatingId(null);
    }
  };
                  ...
                  <div className="order-row-content">
                    <div className="order-products-preview">
                      ...
                      <div>
                        ...
                        <span>{order.items?.[0]?.variantName ? `${order.items[0].variantName} · ` : ''}{itemCount} sản phẩm{(order.items?.length || 0) > 1 ? ` · và ${(order.items?.length || 1) - 1} mặt hàng khác` : ''}</span>
                      </div>
                    </div>
                    ...
                    <div className="order-shipping-meta">
                      ...
                      <span><Package size={15} /> {order.carrier && order.trackingCode ? `${order.carrier} · ${order.trackingCode}` : 'Chưa có thông tin vận chuyển'}</span>
                    </div>
                  ...
                  <div className="order-row-footer">
                    {isManager && (
                      <div className="manager-order-controls">
                        ...
                        <label>
                          <Truck size={14} />
                          <input value={carrierDrafts[order.orderId] || ''} onChange={(event) => setCarrierDrafts((current) => ({ ...current, [order.orderId]: event.target.value }))} placeholder="Hãng vận chuyển" />
                        </label>
                        <label>
                          <Package size={14} />
                        ...
                        <button type="button" title="Lưu thông tin vận chuyển" disabled={updatingId === order.orderId} onClick={() => updateShipping(order.orderId)}><Save size={14} /></button>
                      </div>
                    )}
