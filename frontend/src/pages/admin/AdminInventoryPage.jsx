...
export default function AdminInventoryPage({ user }) {
  ...
  const inventoryPagination = usePagination(
    visibleProducts,
    12,
    `${categoryFilter || 'all'}|${stockFilter}|${threshold}|${appliedSearch}`
  );
        ...
        <div className="admin-inventory-inner">
          ...
          <section className="admin-inventory-panel">
            ...
            {loading && !products.length ? <div className="admin-inventory-loading"><span /><span /><span /><span /></div> : visibleProducts.length ? (
              <div className="admin-inventory-table-wrap"><table className="admin-inventory-table"><thead><tr><th>Sản phẩm</th><th>Danh mục</th><th>Tồn hiện tại</th><th>Tình trạng</th><th>Giá trị kho</th><th aria-label="Thao tác" /></tr></thead><tbody>{inventoryPagination.pageItems.map((product) => {
                const stock = Number(product.stockQuantity || 0);
            ...
            <Pagination {...inventoryPagination} onPageChange={inventoryPagination.setPage} label="sản phẩm" />
            <footer className="admin-inventory-footer">Ngưỡng cảnh báo tồn kho dưới {threshold}</footer>
          </section>
