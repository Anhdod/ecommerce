...
const statusOrder = ['ALL', 'PENDING', 'CONFIRMED', 'PROCESSING', 'SHIPPING', 'DELIVERED', 'CANCELLED'];
const statusMeta = {
  PENDING: { label: 'Chờ xác nhận', icon: Clock3, next: 'CONFIRMED', action: 'Xác nhận đơn' },
  CONFIRMED: { label: 'Đã xác nhận', icon: CheckCircle2, next: 'PROCESSING', action: 'Đóng gói' },
  PROCESSING: { label: 'Đang đóng gói', icon: PackageOpen, next: 'SHIPPING', action: 'Bắt đầu giao' },
...
export default function AdminOrdersPage({ user }) {
  ...
  const [carrierDrafts, setCarrierDrafts] = useState({});
  ...
  const loadData = async () => {
    try {
      ...
      setCarrierDrafts(Object.fromEntries(nextOrders.map((order) => [order.orderId, order.carrier || ''])));
      setDeliveryDateDrafts(Object.fromEntries(nextOrders.map((order) => [order.orderId, order.estimatedDeliveryDate || ''])));
    } catch (error) {
  ...
  const persistShipping = async (orderId) => {
    const carrier = carrierDrafts[orderId]?.trim();
    const trackingCode = trackingDrafts[orderId]?.trim();
    if (!carrier || !trackingCode) {
      showMessage('Vui lòng nhập hãng vận chuyển và mã vận đơn.', 'error');
      return false;
    }
    await api(`/api/orders/${orderId}/shipping`, {
      method: 'PUT',
      body: { carrier, trackingCode, estimatedDeliveryDate: deliveryDateDrafts[orderId] || null },
    });
    return true;
  };
  ...
  const updateShipping = async (orderId) => {
    try {
      setBusyId(orderId);
      const saved = await persistShipping(orderId);
      if (!saved) return;
      showMessage('Đã lưu thông tin vận chuyển.');
      await loadData();
    } catch (error) {
      showMessage(error?.message || 'Không lưu được thông tin vận chuyển.', 'error');
    } finally {
      setBusyId(null);
    }
  };
  ...
  const advanceOrder = async (order) => {
    ...
    try {
      ...
      if (order.status === 'PROCESSING') {
        const saved = await persistShipping(order.orderId);
        if (!saved) return;
      }
              ...
              <><div className="admin-orders-table-wrap">
                <table className="admin-orders-table">
                  ...
                  <tbody>{orderPagination.pageItems.map((order) => {
                    ...
                    const missingShipping = order.status === 'PROCESSING'
                      && (!carrierDrafts[order.orderId]?.trim() || !trackingDrafts[order.orderId]?.trim());
                    const disabled = busyId === order.orderId || (order.status === 'PENDING' && onlineUnpaid) || missingShipping;
                    const canCancel = ['PENDING', 'CONFIRMED', 'PROCESSING'].includes(order.status) && payment?.status !== 'PAID';
                    return (
                      <tr key={order.orderId}>
                        <td><div className="admin-order-product"><span>{firstItem?.imageUrl ? <img src={assetUrl(firstItem.imageUrl)} alt={firstItem.productName} /> : <ShoppingBag size={18} />}</span><div><strong>#{order.orderId}</strong><small>{firstItem?.productName || 'Đơn hàng'}{firstItem?.variantName ? ` · ${firstItem.variantName}` : ''} · {itemCount} sản phẩm</small><i>{formatDate(order.createdAt)}</i></div></div></td>
                        ...
                        <td>{order.status === 'PROCESSING' || order.status === 'SHIPPING' ? <div className="admin-shipping-editor"><div><input value={carrierDrafts[order.orderId] || ''} onChange={(event) => setCarrierDrafts((current) => ({ ...current, [order.orderId]: event.target.value }))} placeholder="Hãng vận chuyển" /><input value={trackingDrafts[order.orderId] || ''} onChange={(event) => setTrackingDrafts((current) => ({ ...current, [order.orderId]: event.target.value }))} placeholder="Mã vận đơn" /><button type="button" title="Lưu thông tin vận chuyển" disabled={busyId === order.orderId} onClick={() => updateShipping(order.orderId)}><Save size={14} /></button></div><input type="date" title="Ngày giao dự kiến" min={new Date().toISOString().slice(0, 10)} value={deliveryDateDrafts[order.orderId] || ''} onChange={(event) => setDeliveryDateDrafts((current) => ({ ...current, [order.orderId]: event.target.value }))} /></div> : <span className="admin-tracking-code">{order.carrier && order.trackingCode ? `${order.carrier} · ${order.trackingCode}` : 'Chưa cần vận đơn'}</span>}</td>
