...
export default function OrderPaymentPage({ user }) {
  ...
  const selectProof = (event) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setError('Ảnh biên lai không được lớn hơn 5 MB.');
      event.target.value = '';
      return;
    }
    setProofFile(file);
  ...
  const submitTransferProof = async (event) => {
    event.preventDefault();
    if (!proofFile) {
    ...
    try {
      ...
      await api(`/api/payments/order/${orderId}/proof`, { method: 'POST', body: formData });
      setProofFile(null);
      setProofPreview('');
      await loadPayment();
    } catch (requestError) {
      setError(requestError?.message || 'Không gửi được biên lai. Vui lòng thử lại.');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const bankTransferOpen = payment?.paymentMethod === 'BANK_TRANSFER'
    && ['PENDING', 'SUBMITTED', 'REJECTED'].includes(payment?.status);

  if (payment?.status !== 'PENDING' && !bankTransferOpen) {
    return (
      <main className="order-payment-page">
        <section className="payment-page-state error">
      ...
      <div className="order-payment-container">
        ...
        {payment.paymentMethod === 'BANK_TRANSFER' ? (
          <section className="bank-transfer-layout">
            ...
            <aside className={`transfer-status-panel ${String(payment.status).toLowerCase()}`}>
              <span>{payment.status === 'REJECTED' ? <XCircle size={22} /> : payment.status === 'SUBMITTED' ? <Check size={22} /> : <Building2 size={22} />}</span>
              <h2>{payment.status === 'SUBMITTED' ? 'Biên lai đang chờ duyệt' : payment.status === 'REJECTED' ? 'Biên lai chưa hợp lệ' : 'Gửi biên lai chuyển khoản'}</h2>
              <p>{payment.status === 'SUBMITTED' ? 'Quản trị viên sẽ kiểm tra giao dịch trước khi xác nhận đơn hàng.' : 'Chụp rõ thông tin giao dịch sau khi chuyển khoản để cửa hàng kiểm tra.'}</p>

              {payment.status === 'REJECTED' && <div className="transfer-rejection"><small>Lý do từ chối</small><strong>{payment.rejectionReason || 'Biên lai chưa đủ thông tin.'}</strong></div>}

              {(proofPreview || payment.proofImageUrl) && (
                <button className="transfer-proof-preview" type="button" onClick={() => window.open(proofPreview || assetUrl(payment.proofImageUrl), '_blank')} title="Mở ảnh biên lai">
                  <img src={proofPreview || assetUrl(payment.proofImageUrl)} alt="Biên lai chuyển khoản" />
                </button>
              )}

              {payment.status === 'SUBMITTED' ? (
                <div className="transfer-submitted"><small>Đã gửi lúc</small><strong>{payment.submittedAt ? new Date(payment.submittedAt).toLocaleString('vi-VN') : 'Vừa xong'}</strong></div>
              ) : (
                <form className="transfer-proof-form" onSubmit={submitTransferProof}>
                  <label className="transfer-proof-picker">
                    <ImagePlus size={17} />
                    <span>{proofFile ? proofFile.name : 'Chọn ảnh biên lai'}</span>
                    <input type="file" accept="image/jpeg,image/png,image/webp" onChange={selectProof} />
                  </label>
                  {error && <p className="transfer-proof-error">{error}</p>}
                  <button className="transfer-proof-submit" type="submit" disabled={processing}>
                    <Upload size={15} /> {processing ? 'Đang gửi...' : payment.status === 'REJECTED' ? 'Gửi lại biên lai' : 'Gửi biên lai'}
                  </button>
                </form>
              )}
