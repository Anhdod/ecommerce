...
const statusInfo = {
  ...
  SUBMITTED: { label: 'Biên lai đang chờ duyệt', icon: ReceiptText },
                ...
                return (
                  <motion.article className="payment-row" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .035 }} key={payment.paymentId}>
                    ...
                    <div className="payment-row-actions">
                      ...
                      {view === 'mine' && ['PENDING', 'SUBMITTED', 'REJECTED'].includes(payment.status) && payment.paymentMethod === 'BANK_TRANSFER' && <Link className="payment-action-primary" to={`/checkout/payment/${payment.orderId}`}><Landmark size={16} /> {payment.status === 'REJECTED' ? 'Gửi lại biên lai' : payment.status === 'SUBMITTED' ? 'Xem biên lai' : 'Hướng dẫn chuyển khoản'}</Link>}
                      {view === 'admin' && payment.status === 'SUBMITTED' && payment.paymentMethod === 'BANK_TRANSFER' && <Link className="payment-action-primary" to="/admin/payments"><Check size={16} /> Kiểm tra biên lai</Link>}
